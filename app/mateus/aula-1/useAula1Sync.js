...
const TOTAL_SECTIONS = 7; // 0-6

export function useAula1Sync(role = 'viewer') {
  // Session state
  ...
  useEffect(() => {
    if (!auth) return; // Firebase not initialized (build/SSR)
  ...
  useEffect(() => {
    ...
    if (role === 'presenter') {
      // Notes (sorted client-side to avoid composite index)
      ...
      // Card responses (sorted client-side to avoid composite index)
      ...
      updateDoc(doc(db, 'sessions', LESSON_ID), {
        ...
        // Session doc may not exist yet — create it
