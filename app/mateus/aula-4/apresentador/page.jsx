...
function SectionNav({ currentSection, totalSections, prevSection, nextSection, sessionStatus, startSession, endSession, resetSession }) {
  ...
  return (
    <WarmCard>
      ...
      <p className="text-lg font-semibold mb-1" style={{ fontFamily: 'var(--font-playfair)', color: colors.charcoal }}>
        {section?.title || ''}
      </p>
      {section?.subtitle && (
        <p className="text-xs mb-4 italic" style={{ color: colors.graphiteLight }}>
          {section.subtitle}
        </p>
      )}
