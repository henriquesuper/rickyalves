// Hook de sincronização para Lição 8
...
const SOCKET_ROOM = 'apologetica-licao-8';
...
export function useLicao8Sync(role = 'viewer') {
    // Estado do slide atual (0 = Quiz, 1-32 = slides)
    ...
    const fetchStatus = useCallback(async () => {
        ...
        try {
            const response = await fetch(`${API_BASE}?action=status&lesson=licao-8`);
    ...
    useEffect(() => {
        ...
        if (role === 'participant') {
            const savedUserId = localStorage.getItem('licao8-userId');
            if (savedUserId) {
                setUserId(savedUserId);
            }

            const savedResponses = localStorage.getItem('licao8-quiz-responses');
            if (savedResponses) {
                try {
        ...
        if (isDevelopment) {
            const initSocket = async () => {
                try {
                    ...
                    console.log(`[Lição 8] Conectando a ${socketHost}`);
                    const socket = io(socketHost, {
                        query: { room: SOCKET_ROOM, role }
                    });

                    socket.on('connect', () => {
                        console.log(`[Lição 8] Conectado como ${role}`);
                        setConnected(true);
                    });

                    socket.on('disconnect', () => {
                        console.log('[Lição 8] Desconectado');
                        setConnected(false);
                    });

                    // Estado inicial
                    socket.on('presentation-state', (data) => {
                        console.log('[Lição 8] Estado recebido:', data);
                        setCurrentSlide(data.currentSlide);
                        setCurrentQuizQuestion(data.currentQuizQuestion);
                        setQuizResponses(data.quizResponses || {});
                        setAttendance(data.attendance || 0);
                    ...
                    socket.on('live-reaction', (reactionData) => {
                        console.log('[Lição 8] Reação recebida:', reactionData);
    ...
    const goToSlide = useCallback(async (slideNumber) => {
        ...
        if (isDevelopment && socketRef.current) {
            socketRef.current.emit('change-slide', { slide: slideNumber, lesson: 'licao-8' });
        } else {
            try {
                await fetch(API_BASE, {
                    ...
                    body: JSON.stringify({
                        action: 'change-slide',
                        data: { slide: slideNumber, lesson: 'licao-8' }
                    })
                });
            } catch (error) {
    ...
    const goToQuizQuestion = useCallback(async (questionNumber) => {
        ...
        if (isDevelopment && socketRef.current) {
            socketRef.current.emit('quiz-question-change', { question: questionNumber, lesson: 'licao-8' });
        } else {
            try {
                await fetch(API_BASE, {
                    method: 'POST',
                    ...
                    body: JSON.stringify({
                        ...
                        data: { question: questionNumber, lesson: 'licao-8' }
                    })
                });
            } catch (error) {
    ...
    const respondToQuiz = useCallback(async (questionNumber, response) => {
        ...
        localStorage.setItem('licao8-quiz-responses', JSON.stringify(newResponses));
            ...
            try {
                await fetch(API_BASE, {
                    ...
                    body: JSON.stringify({
                        ...
                        data: { question: questionNumber, response, lesson: 'licao-8' }
                    })
                });
            } catch (error) {
    ...
    const join = useCallback(async (userName) => {
        ...
        setUserId(newUserId);
        localStorage.setItem('licao8-userId', newUserId);
        localStorage.setItem('licao8-userName', userName);

        if (isDevelopment && socketRef.current) {
            socketRef.current.emit('join', { userName, lesson: 'licao-8' });
        } else {
            try {
                await fetch(API_BASE, {
                    method: 'POST',
                    ...
                    body: JSON.stringify({
                        action: 'join',
                        data: { userName, userId: newUserId, lessonId: 'licao-8' }
                    })
                });
            } catch (error) {
    ...
    const logout = useCallback(async () => {
        // Limpar localStorage
        localStorage.removeItem('licao8-userId');
        localStorage.removeItem('licao8-userName');
        localStorage.removeItem('licao8-quiz-responses');

        // Emitir evento de saída
        if (isDevelopment && socketRef.current) {
            socketRef.current.emit('leave', { lessonId: 'licao-8', userId });
        } else {
            try {
                await fetch(API_BASE, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ action: 'leave', data: { lessonId: 'licao-8', userId } })
                });
            } catch (error) {
    ...
    const react = useCallback(async (reactionType, userName) => {
        console.log('[Lição 8] Enviando reação:', reactionType, 'de', userName);
        ...
        if (isDevelopment && socketRef.current) {
            ...
            try {
                await fetch(API_BASE, {
                    ...
                    body: JSON.stringify({
                        action: 'reaction',
                        data: { type: reactionType, userName, lessonId: 'licao-8' }
                    })
                });
            } catch (error) {
    ...
    const vote = useCallback(async (pollId, optionIndex) => {
        if (isDevelopment && socketRef.current) {
            socketRef.current.emit('vote', { pollId, optionIndex, lessonId: 'licao-8' });
        } else {
            try {
    ...
    const endPoll = useCallback(async () => {
        ...
        if (isDevelopment && socketRef.current) {
            socketRef.current.emit('end-poll', { lessonId: 'licao-8' });
        } else {
            try {
                await fetch(API_BASE, {
                    method: 'POST',
